...
export default function VoltageChart({ data, mode, containerWidth }) {
  ...
  const { inputPts, outputPts, gridLines, xLabels, xPos, yPos } = useMemo(() => {
    if (!data || data.length === 0) return { inputPts: [], outputPts: [], gridLines: [], xLabels: [] };
    ...
    return { inputPts, outputPts, gridLines, xLabels, xPos, yPos };
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: e => {
      ...
      setHoverIdx(Math.round(ratio * ((data?.length || 1) - 1)));
    },
  ...
  const inputLine = smoothPath(inputPts);
  const outputLine = smoothPath(outputPts);
  const inputFill = fillPath(inputPts, chartBottom);
  const outputFill = fillPath(outputPts, chartBottom);

  const hoverData = hoverIdx != null && data[hoverIdx] ? data[hoverIdx] : null;
  const hoverX = hoverIdx != null && xPos ? xPos(hoverIdx) : null;
  const hoverInY = hoverData && yPos ? yPos(hoverData.inputV) : null;
  const hoverOutY = hoverData && yPos ? yPos(hoverData.outputV) : null;

  // Tooltip position
  const ttLeft = hoverX != null ? Math.max(PAD_L, Math.min(hoverX - 60, W - 134)) : 0;
  const ttTop = 12;
      ...
      <Svg width={W} height={H}>
        ...
        {gridLines.map((g, i) => (
          <React.Fragment key={i}>
            ...
            <SvgText x={PAD_L - 6} y={g.y + 4} textAnchor="end"
              fontSize={10} fill={COLORS.text3}>{g.v}V</SvgText>
          </React.Fragment>
        ))}

        {xLabels.map((xl, i) => (
          <SvgText key={i} x={xPos(xl.idx)} y={H - 8}
            textAnchor="middle" fontSize={10} fill={COLORS.text3}>
            {xl.label}
          </SvgText>
        ))}
        ...
        {hoverData && hoverX != null && (
          <>
            ...
            <Circle cx={hoverX} cy={hoverInY} r={5} fill="white"
              stroke={COLORS.inputColor} strokeWidth={2.5} />
            <Circle cx={hoverX} cy={hoverOutY} r={5} fill="white"
              stroke={COLORS.outputColor} strokeWidth={2.5} />
          </>
        )}
      </Svg>

      {hoverData && (
        <View style={[styles.tooltip, { left: ttLeft, top: ttTop }]}>
          <Text style={styles.ttTime}>{getLabel(hoverData, mode)}</Text>
          <View style={styles.ttRow}>
            <View style={[styles.ttDot, { backgroundColor: COLORS.inputColor }]} />
            <Text style={styles.ttLabel}>Input</Text>
            <Text style={styles.ttVal}>{hoverData.inputV.toFixed(1)} V</Text>
          </View>
          <View style={styles.ttRow}>
            <View style={[styles.ttDot, { backgroundColor: COLORS.outputColor }]} />
            <Text style={styles.ttLabel}>Output</Text>
            <Text style={styles.ttVal}>{hoverData.outputV.toFixed(1)} V</Text>
          </View>
        </View>
      )}
    </View>
